...
/**
 * class for client to supply their own Filter kernel. This method applies the supplied kernel to
 * the supplied model.ImageModel by calling applyFilter, Enabling clients to use their own Filter
 * kernel. Prevents accidentally supplying incorrect kernel type to Blur, Sharpen, Sepia, GreyScale,
 * and other such default filter types.
 */
public class SupplyFilter implements IKernel {

  private final double[][] supplyKernel;

  /**
   * Constructor allows users to input their own Filter Kernel.
   */
  public SupplyFilter(double[][] supplyKernel) {
    if (supplyKernel.length % 2 == 0 || supplyKernel[0].length != supplyKernel.length) {
      throw new IllegalArgumentException(
          "Filter kernel must be a square 2D array of odd dimensions!");
    } else {
