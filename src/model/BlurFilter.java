...
/**
 * Blur implementation of a Filter Kernel.
 */
public class BlurFilter implements IKernel {

  private final double[][] blurKernel;

  /**
   * Blur filter constructor that initializes the kernel to the values on the assignment page.
   */
  public BlurFilter() {
    double[] column1 = new double[]{0.0625, 0.125, 0.0625};
    double[] column2 = new double[]{0.125, 0.25, 0.125};
    this.blurKernel = new double[][]{column1, column2, column1};
  }
  ...
  public double[][] getKernelArray() {
    ...
    for (int i = 0; i < this.blurKernel.length; i++) {
      for (int j = 0; j < this.blurKernel.length; j++) {
        returnMatrix[i][j] = this.blurKernel[i][j];
      }
    }
    return returnMatrix;
  }
